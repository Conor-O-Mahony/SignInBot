...
async def on_message(message):
  ...
  if message.content.startswith('$allhours'):
    print(str(hours.cell(12,2)))
    print(str(hours.cell(2,3).value))
    for j in range(1,len(hours.row_values(2))):
      embeddict={}
      temp=0
      time1=datetime.datetime.strptime(str(hours.cell(3,j+1).value), '%H:%M:%S')
      print(time1)#lawrencedoesntworkfor
      addtime1=datetime.timedelta(hours=time1.hour, minutes=time1.minute, seconds=time1.second)
      for i in range(2,len(hours.col_values(1))+1):
        if hours.cell(i+1,j+1).value:
          time1=datetime.datetime.strptime(str(hours.cell(i+1,j+1).value), '%H:%M:%S')
          addtime1+=datetime.timedelta(hours=time1.hour, minutes=time1.minute, seconds=time1.second)
          time.sleep(1.8)
          #print(time1)
          #print(addtime1)
      time1hours=round(addtime1.days *24 + addtime1.seconds/3600, 2)
        #print(hours.col_values(j+1)[i])
        #temp+=hours.col_values(j+1)[i]
      embeddict[hours.row_values(2)[j]]=time1hours
      print(embeddict)
    ...
    for key, value in embeddict.items() :
        #print (key, value)
      #for j in range(2,len(hours.row_values(2))):
        #temp1=str(hours.cell(2,j).value)
